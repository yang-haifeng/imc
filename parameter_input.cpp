...
double ParameterInput::GetReal(std::string block, std::string name) {
  InputBlock* pb;
  InputLine* pl;
  std::stringstream msg;
...
double ParameterInput::GetOrAddReal(std::string block, std::string name, double def_value) {
  InputBlock* pb;
  std::stringstream ss_value;
...
double ParameterInput::SetReal(std::string block, std::string name, double value) {
  InputBlock* pb;
  std::stringstream ss_value;
...
void ParameterInput::RollbackNextTime() {
  ...
  while (pb != NULL) {
    if (pb->block_name.compare(0, 6, "output") == 0) {
      ...
      next_time = static_cast<double>(atof(pl->param_value.c_str()));
...
void ParameterInput::ForwardNextTime(double mesh_time) {
  InputBlock *pb = pfirst_block;
  InputLine* pl;
  double next_time;
  double dt0, dt;
  ...
  while (pb != NULL) {
    if (pb->block_name.compare(0, 6, "output") == 0) {
      ...
      dt0 = static_cast<double>(atof(pl->param_value.c_str()));
