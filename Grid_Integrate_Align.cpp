...
Vector Grid::Integrate(double x0, double y0, double z0, 
  ...
  //std::cout<<std::setprecision(6)<<x/AU<<'\t'<<y/AU<<'\t'<<z/AU<<'\t'<<ir<<"\t"<<it<<std::endl;
  while (this->isInDomain(x,y,z)){
    //irs = ir; its=it;
    this->findCell(x,y,z,ir,it);
    rho = this->get_density(ir, it);
    this->moveOneCell(x,y,z, nx,ny,nz, ds, ir,it);
    dtau = rho * ds * kappa_ext;

    if (dtau>0.1) ds = 0.1/rho/kappa_ext;

    dSe = this->calcEmission(ir,it, x,y,z, nx,ny,nz);

    if(ScaFlag) dSs=this->calcScattering(ir,it, x,y,z, nx,ny,nz);
    else dSs[0]=dSs[1]=dSs[2]=dSs[3]=0.;

    M = this->calcExtinction(ir,it, x,y,z, nx,ny,nz);
    ...
    /*
    std::cout<<"-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-="<<std::endl;
    std::cout<<std::setprecision(7)<<x/AU<<'\t'<<ds/AU<<'\t'<<S[0]<<'\t'<<rho<<std::endl;
    std::cout<<"-=-=-=-=-=-=-=-=-=-=-=-=-="<<rho*ds<<"-=-=-=-=-=-=-=-=-=-=-=-=-="<<std::endl;
    for(int i=0; i<4; i++){
     for(int j=0; j<4; j++){
       std::cout<<T[i*4+j]<<"\t";
     }
    ...
    */
    //std::cout<<std::setprecision(16)<<x<<'\t'<<y<<'\t'<<z<<'\t'<<ds/AU<<"\t"<<ir<<"\t"<<it<<std::endl;
  ...
  //std::cout<<std::setprecision(16)<<nx<<'\t'<<ny<<'\t'<<nz<<'\t'<<std::endl;
  return S;
}
